...
- [x] **Ablative Plating Wear:** Micro-meteoroids and projectile hits permanently degrade maximum armor value. *(Integrated 2026-05-26 — new `player.plating_wear` int field on `ShipEntity` (mirrored in `include/entities.h` and the inline duplicate in `src/game.c`), initialised to 0 in `game_init()` only (NOT reset_player — wear persists across deaths within a run). Six combat-impact `lives--` sites now bump wear: asteroid collision, UFO body, UFO bolt, Ascian bolt, and Lictor bolt each add +1; Rust-Weaver corrosive spit adds +2 because the lore explicitly says it "dissolves plating". Each tick spawns an amber `PLATING -1` (or `-2`) EventFloat above the explosion. New constant `FUEL_PLATING_WEAR_RATE = 0.005f` hooks into the existing passive-drain block in `update_player_physics()` — drain becomes `BASE + relics * RELIC_RATE + wear * 0.005f` (then Solar Flare multiplier stacks on top). At 16 wear points the wear penalty equals the base drain, effectively doubling fuel burn for a fresh hull. At wear == `PLATING_WEAR_HEAVY_THRESH (15)` Cugel-9 fires `"HULL PLATING CRITICAL. REACTOR SEAL IS NOW DECORATIVE."` exactly once. Permadeath-feel without lethal escalation. Clean build, 343,766-byte exe (+1,536 bytes), no new warnings.)*
- [x] **Calibration Codes:** Found memory chips (equivalent to Scrolls of Enchant Armor/Weapon) can be compiled to boost weapon damage (`+1 CAL`) or armor deflection.
- [x] **Void-Rust Anomalies:** Radioactive gas clouds that slowly dissolve ship hull integrity and erode weapon accuracy. *(Integrated 2026-05-27 — completes the §4 Plating Durability & Calibration sub-section. New `VoidRustCloudEntity` typedef mirrored in BOTH `include/entities.h` AND the inline duplicate in `src/game.c` (same dual-struct wart all ShipEntity-adjacent items document). New `#define MAX_VOID_RUST_CLOUDS 4` likewise mirrored. New `player.void_rust_damage_t` float field on `ShipEntity` (both header + inline copy), initialised to `VOID_RUST_DAMAGE_INTERVAL = 2.5f` in `reset_player()` and `game_init()` so the next entry to a cloud has a fresh damage timer. File-static state: `void_rust_clouds[MAX_VOID_RUST_CLOUDS]` pool, `void_rust_spawn_timer`, `void_rust_next_iv` (jittered 45–80 s interval), `player_in_void_rust` flag (recomputed per frame in `update_progression()` and consumed by both the damage-tick path and the bullet-fire jitter hook), `void_rust_in_cloud_prev` (last-frame flag for entry-only quip). Spawn (`spawn_void_rust_cloud()`) rolls a 120–180 u radius, 15–40 u/s drift on a random angle, 28 s lifetime, placed 800–1400 u from player on a random bearing — clouds appear as drifting hazards in the navigation arc, never on top of the ship. Periodic spawn in `update_progression()` gated on Zone 2+ (Home Space and Inner Belt are clean). Containment check per frame walks the pool and sets `player_in_void_rust` if any active cloud's radius+player.radius overlaps; on entry transition spawns sickly yellow-green `VOID RUST` event float (170,220,70) + `cugel9_say("RADIOACTIVE GAS POCKET. AIM DEGRADED. PLATING DISSOLVING. NOMINAL.")`. Two effects while inside: (1) **plating wear tick** — `update_player_physics()` decrements `player.void_rust_damage_t` and on rollover `player.plating_wear += 1` (reuses Item 38 counter, so Calibration Codes (Item 39) can reverse this wear too), spawns amber `PLATING -1` float, plays `SFX_EXPLOSION_SM`, and fires the heavy-threshold Cugel-9 quip if wear hits 15; (2) **muzzle jitter** — at every bullet-fire site (forward triple-shot AND rear gun) the muzzle angle picks up an independent `±VOID_RUST_ACCURACY_JITTER = 0.12 rad` random offset, so each bolt scatters realistically and the triple-shot spread still reads as three distinct shots. Outside any cloud `void_rust_damage_t` is held at `VOID_RUST_DAMAGE_INTERVAL` so re-entry doesn't fire an instant tick. Render block in `render_entities()` before asteroids: three concentric arc rings at 100 % / 78 % / 52 % of cloud radius, each a 14-segment line loop with `±6 u` per-vertex wobble keyed off the cloud's animation phase × per-ring multiplier (so rings shimmer at different frequencies, giving a churning gas-pocket feel); sickly radioactive yellow-green (170,220,70) at 0.55/0.42/0.30 ring alpha; plus 6 deterministic "speck" particles per cloud anchored to cloud index so they persist frame-to-frame; alpha envelope = `fade_in(1.2 s)` × `fade_out(3.0 s)` so clouds breathe in and dissipate smoothly rather than popping. Constants: `VOID_RUST_CLOUD_RADIUS_MIN/MAX = 120/180 u`, `VOID_RUST_CLOUD_LIFE = 28 s`, `VOID_RUST_SPAWN_MIN/MAX_IV = 45/80 s`, `VOID_RUST_MIN_ZONE = 2`, `VOID_RUST_DAMAGE_INTERVAL = 2.5 s`, `VOID_RUST_ACCURACY_JITTER = 0.12 rad`, `VOID_RUST_SPAWN_DIST_MIN/MAX = 800/1400 u`. Clean build verified via `mingw32-make` from worktree, 349,244-byte exe (+2,773 bytes over Calibration Codes baseline), no new warnings beyond the five pre-existing ones.)*
